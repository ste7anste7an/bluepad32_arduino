...
#define DEFAULT_SENSOR COLOR_MATRIX
#include <EEPROM.h>
...
// 3x3 matrix
byte led_mapping[9] = { 2, 1, 0, 3, 4, 5, 8, 7, 6 };
...
byte cmd_map(byte lego_led_nr, byte np_led_nr) {
  //Serial.println((String)"lego_led_nr="+lego_led_nr+" np_led_nr="+np_led_nr);
  if (np_led_nr >= 0 && np_led_nr < 100 && lego_led_nr >= 0 && lego_led_nr < 11) {
    sensor_conf.led_mapping[lego_led_nr] = np_led_nr;
    return 1;
  } else {
    return 0;
  }
...
void functShow(CmdParser *myParser) {
  ...
  for (int i = 0; i < 9; i++) { Serial.print((String)sensor_conf.led_mapping[i] + " "); }
  Serial.println();
  for (int i = 0; i < 11; i++) {
...
void functNeopixel(CmdParser *myParser) {
  ...
  if (myParser->equalCmdParam(1, "SET")) {
...
void functHelp(CmdParser *myParser) {
  ...
  Serial.println("set map     <lego_led> <neo_led>    : maps lego led number <lego_led> to neopixel led number <neo_led>");
...
void neopixel_callback(byte buf[], byte s) {
  ...
  if (memcmp(old_values, buf, 9) != 0) {
    ...
    for (int i = 0; i < 9; i++) {
      ...
      neopixel_strip->setPixelColor(sensor_conf.led_mapping[i], colr, colg, colb);
    }
    neopixel_strip->show();
  }
}
...
void setup() {
  ...
  Serial.println("LOG: BluePad32 SPIKEv3 3x3 led matrix, see https://github.com/antonvh/PUPRemote/blob/main/examples/bluepad/spike3/README.md");
