...
#endif
    if (Serial2.available()) {
      Serial.print("LOG: received data from hub: ");
      byte b = read_byte();
      Serial.println(b, HEX);
      if (b == BYTE_ACK) {
        Serial.println("LOG: ACK received from SPIKE3 hub.");
        Serial2.end(); 
        ...
        num_tries++;
        if (num_tries < 2) {
          Serial.printf("LOG: Number of retries: %d.\n", num_tries);
        } else {
          return 0;
          break;
        }
      ...
      Serial.println("ERROR: ACK timed out");
      Serial2.end();
      pinMode(txpin, OUTPUT);
      digitalWrite(txpin, HIGH);
      delay(200);
