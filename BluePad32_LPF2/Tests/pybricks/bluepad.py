...
def word_to_bytes(w):
    return ustruct.unpack('2b',ustruct.pack('h',w))
...
def signed_to_unsigned(b):
    return ustruct.unpack('B',ustruct.pack('b',b))[0]
...
class BluePad:
    ...
    def gamepad(self):
        ...
        gp_words=self.pup.read(self.cur_mode)
        arr=[]
        for i in range(2):
            b1,b2=word_to_bytes(gp_words[i])
            arr.append(from_signed(b1))
            arr.append(from_signed(b2))
        b1,b2=word_to_bytes(gp_words[2])
        arr.append(signed_to_unsigned(b1))
        arr.append(signed_to_unsigned(b2))
        return arr
    ...
    def neopixel_init(self,nr_leds,pin):
        ...
        """
        leds=[0]*8
        leds[0]=CONFIG+nr_leds*256
        leds[1]=pin
        self.cur_mode=0
        self.pup.write(self.cur_mode,leds)
    ...
    def neopixel_fill(self,color,write=True):
        ...
        global cur_mode
        leds=[0]*8
        leds[0]=(FILL|WRITE if write else FILL)+color[0]*256
        leds[1]=color[1]+color[2]*256
        self.cur_mode=0
        self.pup.write(self.cur_mode,leds)
    ...
    def neopixel_set(self,led_nr,color,write=True):
        ...
        leds=[0]*8
        # start led = 1
        leds[0]=(SET|WRITE if write else FILL) + 1 *256 
        if led_nr>=self.nr_leds:
            print("error neopixel_set: led_nr larger than number of leds!")
            r=None
        else:
            leds[1]=led_nr+color[0]*256
            leds[2]=color[1]+color[2]*256
        self.cur_mode=0
        self.pup.write(self.cur_mode,leds)
    ...
    def neopixel_set_multi(self,start_led,nr_led,led_arr,write=True):
        ...
        """
        leds=[0]*8
        leds[0]=(SET|WRITE if write else FILL) + nr_led*256
        if nr_led>4:
    ...
    def servo(self,servo_nr,pos):
        ...
        s[servo_nr]=pos%180
        self.cur_mode=1
        self.pup.write(self.cur_mode,s)
